...
首先，通过gate = act_fn(gate_proj(x))得到门控信号（范围通常为[0, 1]或正数）；

其次，通过value = up_proj(x)得到要被控制的信息通道；

接着，使用gated = gate * value应用门控，控制信息强度（本质上就是加权）；
...
![image.png](../images/s2-advanced/3.png)
